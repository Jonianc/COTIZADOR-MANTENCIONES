...
<?php
    ...
    public function render_quotes_page() {
        ...
        if (isset($_GET['view'])) {
            ...
            ACPDF_PDF::output_pdf_from_log($log[$index], true);
            exit;
        }
          ...
          <table class="widefat striped">
            ...
            <tbody>
              ...
              foreach (array_reverse($log, true) as $index => $entry) {
                  ...
                  $has_rows = true;
                  $has_payload = !empty($entry['payload']);
                  $view_url = $has_payload
                      ? wp_nonce_url(admin_url('options-general.php?page=acpdf-quotes&view='.$index), 'acpdf_view_quote_'.$index)
                      : '';
                  ?>
                  <tr>
                    ...
                    <td>
                      <?php if ($has_payload) : ?>
                        <a href="<?php echo esc_url($view_url); ?>" target="_blank" rel="noopener noreferrer">Ver PDF</a>
                      <?php else : ?>
                        <span class="description">No disponible</span>
                      <?php endif; ?>
                    </td>
                  </tr>
                  <?php
